...
% abundanceDir = "D:\ZooPlanktonOutputs\OrganismDensityCalc_2024-02-27 20-55-00.071_2024-02-28 00-21-00.363(7)";
abundanceDir = "D:\ZooPlanktonOutputs\03_02_DensityCalc_bulk";

inputFiles = dir(abundanceDir);
...
allDensities = [];
allTimes = [];
    outputTitle = "BatchProcess_niche_bulk";
...
for Y = 3:length(inputFiles)
%% Begin Algorithm
% for j = 1:length(abundanceCalcFiles)
tempDir = inputFiles(Y).name;
if exist(fullfile(abundanceDir,tempDir)) ~= 7
    continue
end
tempOutputDir = fullfile(baseOutputDir,strcat(tempDir,"_niche"));
abundanceDirTable = struct2table(dir(fullfile(abundanceDir,tempDir)));
...
baseDensityTable = readtable(fullfile(abundanceDir,tempDir,"densityOutput.csv"));
...
% end
    ...
    tempTime = extract(abundanceCalcFiles(z),digitsPattern(4)+"-"+digitsPattern(2)+"-"+digitsPattern(2)+ " " + digitsPattern(2)+"-"+digitsPattern(2)+"-"+digitsPattern(2)+"."+digitsPattern(3));
    ...
    tTotal = toc(tStart);
        ...
        outputDir = fullfile(abundanceDir,tempDir,abundanceCalcFiles(z));
        ...
        classCounts = getClassDists_v2(tempOutputDir,netTable,parallelPool,abundanceCalcFiles(z),outputDir);
    ...
    %%
    fullOutputTable = cat(1,fullOutputTable,outputTable);
    tTotalEnd = toc(tStart);
    writetable(struct2table(outputTable),fullfile(tempOutputDir,"densityOutput.csv"))
