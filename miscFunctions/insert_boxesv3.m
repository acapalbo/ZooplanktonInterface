...
function boxedVideo = insert_boxesv3(raw_video,csvDir)
    ...
    for z = 1:size(raw_video,3)
        raw_img = raw_video(:,:,z);
        boxed_image = raw_img;
        if exist(strcat(csvDir,"/Frame_",num2str(z),".csv"))
            ...
            for i = 1:detections
                pos = uint16([boxes(i,1)+1, boxes(i,2)+1, boxes(i,3), boxes(i,4)]);
                boxed_image = insertShape(boxed_image, "rectangle", pos, ...
                    ShapeColor=("red"),LineWidth = 4);%,Opacity=0.5);
            end
        else
            boxed_image = cat(3,boxed_image,boxed_image,boxed_image);
        end
        if exist("boxedVideo")
