...
function dataSetFilePath = segment_objects(vid,BW,h_vars,videoPath,saveTrashImages,minLength,minWidth,outputDir)
    ...
    mkdir(strcat(outputDir,"/TableInfo_",string(datetime("today","Format","MM_dd_yy"))));
    mkdir(strcat(outputDir,"/NonRelevantObjects_",string(datetime("today","Format","MM_dd_yy"))));
    mkdir(strcat(outputDir,"/DataSet_",string(datetime("today","Format","MM_dd_yy"))));
    ...
    for k = 1:frame_num
        ...
        writetable(infoTable,strcat(outputDir,"\TableInfo_",string(datetime("today","Format","MM_dd_yy")),"\Frame_",num2str(k),".csv"));
    ...
    dataSetFilePath = strcat(outputDir,"\DataSet_",string(datetime("today","Format","MM_dd_yy")));
    % begin sorting
end